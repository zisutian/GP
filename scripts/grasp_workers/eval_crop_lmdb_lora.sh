...
export MASTER_PORT="${MASTER_PORT:-${GRASP_CROP_EVAL_MASTER_PORT}}"

CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-${GRASP_CROP_EVAL_CUDA_VISIBLE_DEVICES}}"
GPUS="${GPUS:-${GRASP_EVAL_GPUS}}"
export CUDA_VISIBLE_DEVICES

if [[ -z "${CHECKPOINT:-}" ]]; then
  if [[ -z "${EXPERIMENT_NAME:-}" ]]; then
    EXPERIMENT_NAME="$("${PYTHON_BIN}" "${GP_ROOT}/grasp_settings.py" default-name crop)"
  fi
