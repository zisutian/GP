...
export MASTER_PORT="${MASTER_PORT:-${GRASP_DIRECT_EVAL_MASTER_PORT}}"

CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-${GRASP_DIRECT_EVAL_CUDA_VISIBLE_DEVICES}}"
GPUS="${GPUS:-${GRASP_EVAL_GPUS}}"
export CUDA_VISIBLE_DEVICES

if [[ -z "${CHECKPOINT:-}" ]]; then
  if [[ -z "${EXPERIMENT_NAME:-}" ]]; then
    EXPERIMENT_NAME="$("${PYTHON_BIN}" "${GP_ROOT}/grasp_settings.py" default-name direct)"
  fi
  if [[ -z "${WORK_DIR:-}" ]]; then
    WORK_DIR="${GRASP_DIRECT_RUN_ROOT}/${EXPERIMENT_NAME}"
  fi
...
if [[ "${VCOT_PIPELINE}" != "direct_grasp" ]]; then
  echo "Checkpoint pipeline is ${VCOT_PIPELINE}, expected direct_grasp: ${CHECKPOINT}" >&2
  exit 1
fi
...
DATASETS="${DATASETS:-${GRASP_EVAL_DATASETS}}"
OUT_DIR="${OUT_DIR:-${GRASP_DIRECT_RESULT_ROOT}/${EXPERIMENT_NAME}}"
DATA_INDEX_ROOT="${DATA_INDEX_ROOT:-${VCOT_DATA_INDEX_ROOT}}"
VCOT_IOU_THRESHOLD="${VCOT_IOU_THRESHOLD:-${GRASP_EVAL_VCOT_IOU_THRESHOLD}}"
VCOT_ANGLE_THRESHOLD="${VCOT_ANGLE_THRESHOLD:-${GRASP_EVAL_VCOT_ANGLE_THRESHOLD}}"
