...
export MASTER_PORT="${MASTER_PORT:-${GRASP_VCOT_EVAL_MASTER_PORT}}"

CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-${GRASP_VCOT_EVAL_CUDA_VISIBLE_DEVICES}}"
GPUS="${GPUS:-${GRASP_EVAL_GPUS}}"
export CUDA_VISIBLE_DEVICES

if [[ -z "${CHECKPOINT:-}" ]]; then
  if [[ -z "${EXPERIMENT_NAME:-}" ]]; then
    EXPERIMENT_NAME="$("${PYTHON_BIN}" "${GP_ROOT}/grasp_settings.py" default-name vcot)"
  fi
  if [[ -z "${WORK_DIR:-}" ]]; then
    WORK_DIR="${GRASP_VCOT_RUN_ROOT}/${EXPERIMENT_NAME}"
  fi
...
if [[ "${VCOT_PIPELINE}" != "predicted_vcot" ]]; then
  echo "Checkpoint pipeline is ${VCOT_PIPELINE}, expected predicted_vcot: ${CHECKPOINT}" >&2
  exit 1
fi
...
OUT_DIR="${OUT_DIR:-${GRASP_VCOT_RESULT_ROOT}/${EXPERIMENT_NAME}}"
