...
BBOX_LOSS_WEIGHT=${BBOX_LOSS_WEIGHT:-${GRASP_VCOT_BBOX_LOSS_WEIGHT}}
...
BBOX_LOSS_TAG=""
if [[ "${BBOX_LOSS_WEIGHT}" != "1" && "${BBOX_LOSS_WEIGHT}" != "1.0" ]]; then
  BBOX_LOSS_TAG="_lambda${BBOX_LOSS_WEIGHT}"
fi
DEFAULT_EXPERIMENT_NAME="vcot_${TARGET_FRAME_TAG}_lora${USE_LLM_LORA}_lr${LEARNING_RATE}_ep${NUM_TRAIN_EPOCHS}_patch${MAX_DYNAMIC_PATCH}_edge${BBOX_EDGE_EXPAND}_half${MIN_BBOX_HALF_SIZE}_bbox${BBOX_RATIO}${BBOX_LOSS_TAG}"
